...
interface LockedFinanceModalProps {
  isOpen: boolean;
  onClose: () => void;
  featureTitle?: string;
...
export function LockedFinanceModal({
  isOpen,
  onClose,
  featureTitle = 'Bu Bölüm',
  featureIcon,
}: LockedFinanceModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-emerald-500/20 to-teal-600/20 text-4xl shadow-inner ring-1 ring-emerald-500/30">
              {featureIcon || <Sparkles className="h-8 w-8 text-emerald-500" />}
            </div>

            {/* Pill Badge */}
            <div className="mb-3 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-3 py-1 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
